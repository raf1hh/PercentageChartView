...
public class HalfRingModeRenderer extends BaseModeRenderer implements OrientationBasedMode {
    ...
    public HalfRingModeRenderer(IPercentageChartView view) {
        super(view);
        init();
        setup();
    }

    public HalfRingModeRenderer(IPercentageChartView view, TypedArray attrs) {
        super(view, attrs);
        init(attrs);
        setup();
    }
    ...
    public void draw(Canvas canvas) {
        //BACKGROUND
        if (mDrawBackground) {
            canvas.drawArc(mBackgroundBounds, mStartAngle + tweakAngle, 180, false, mBackgroundPaint);
        }

        //BACKGROUND BAR
        if (mDrawBackgroundBar) {
            canvas.drawArc(mCircleBounds, mStartAngle + tweakAngle, 180, false, mBackgroundBarPaint);
        }
    ...
    void updateDrawingAngles() {
        switch (orientation) {
            case ORIENTATION_COUNTERCLOCKWISE:
                mSweepAngle = -(mProgress / DEFAULT_MAX * 180);
                break;
